...
export default function MainCourses() {
    ...
    const manageSearch = (searchValue) => {
        setSearch(searchValue);
    }
    
    useEffect(() => {
        getRequest(`/api/food/main-courses/${search}/search/${getUserLogin()}`)
            .then((response) => setData(response.data))
            .catch((error) => navigate("/"));

           if(data.length > 0) setLoading(false)

    }, [data.length, navigate, search]);
    ...
    return loading ?
        ...
        <>
            ...
            <SearchBar manageSearchFn={manageSearch}/>
